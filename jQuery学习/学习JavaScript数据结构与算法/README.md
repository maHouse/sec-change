...
假设现在要执行recursiveFunction，结果是什么？单单就上述情况而言，他会一直执行下去。因此每个递归函数都必须要有边界条件，即一个不再递归调用的条件（停止点），以防止无限递归。

**JS调用栈大小的限制**

如果忘记加上用以停止函数递归调用的边界条件，会发生什么呢？递归并不会无限制地执行下去；浏览器会抛出错误，也就是所谓的栈溢出错误（stack overflow error）。